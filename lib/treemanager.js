 ...
 * @param {Function} cb Callback with changed name
 */
...
/**
 * Delete node. 
 * Removes directories recursivelly. 
 * 
 * @param {String} nodePath Relative path of the node to be removed
 * @param {Function} cb Callback function
 */
fileModule.prototype.deleteNode = function(nodePath, cb) {
  var file = path.join(this.root, nodePath);
  fs.stat(file, function(err, stat) {
    if(err) {
      cb(new Error ('File to remove do not exist'))
    }
    else {
      if(stat.isDirectory()) {
 ...
 * Move node. 
 * 
 * @param {String} nodePath Relative path of the node to be moved
 * @param {String} nodeParent Relative path of the parent target node
 * @param {Function} cb Callback function
 */
fileModule.prototype.moveNode = function(nodePath, nodeParent, cb) {
  var fileName = nodePath.split('/').pop();
  var newName = path.join(this.root, nodeParent, fileName);
  fs.rename(path.join(this.root, nodePath), newName, function () {
    var newPath = path.join(nodeParent, fileName);
    cb(newPath);
  });
};

fileModule.prototype.readFile = function (file, cb) {
...
// https://gist.github.com/geedew/48a4888e30271b930d33#file-node-rm-rf-async-js
